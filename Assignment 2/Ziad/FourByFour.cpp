...
int new_x, new_y;
Template
class FourByFour : public Board<T>
{
    public:
        FourByFour();
...
Template
class FourByFour_Player : public Player<T>, public FourByFour<T>
{
    public:
...
Template
class FourByFour_Random_Player : public RandomPlayer<T>, public FourByFour<T>
{
    public:
...
FourByFour<T> :: FourByFour()
{
    ...
    for (int i = 0; i < this->rows; i++) {
        ...
        for (int j = 0; j < this->columns; j++) {
            if(i == 0)
            {
                if(j % 2 == 0) this->board[i][j] = 'O';
                else this->board[i][j] = 'X';
            }
            else if(i == 3)
            {
                if(j % 2 == 0) this->board[i][j] = 'X';
...
Template
bool FourByFour<T> :: update_board(int x, int y, T symbol)
{
    if (new_x >= 0 && new_x < this->rows && new_y >= 0 && new_y < this->columns 
    && this->board[new_x][new_y] == ' ' && this->board[x][y] == symbol && ((abs(new_x - x) == 1 && abs(new_y - y) == 0) || ((abs(new_x - x) == 0 && abs(new_y - y) == 1)))
...
Template
void FourByFour<T> :: display_board()
{
    for (int i = 0; i < this->rows; i++) {
        cout << string(40, '-') << endl;
        cout << "| ";
...
Template
bool FourByFour<T> :: is_draw()
{
    return (this->n_moves == 16 && !is_win());
}

Template
bool FourByFour<T> :: game_is_over()
{
...
Template
void FourByFour_Random_Player<T> :: getmove(int &x, int &y)
{
