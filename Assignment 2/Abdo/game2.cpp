...
bool iswin(vector<vector<char>>& board , char z){
        for (int i = 0; i < 6; ++i) {
            for (int j = 0; j < 7; ++j) {
                if (board[i][j] == z && (
                    check_direction(board,i, j, 1, 0) ||  
                    check_direction(board,i, j, 0, 1) ||  
                    check_direction(board,i, j, 1, 1) || 
                    check_direction(board,i, j, 1, -1))) return true;
            }
        }
        return false;
}
...
public:
    randomPlayer(string name, char symbol) : Player<char>(name, symbol) {}
...
class MinimaxPlayer : public Player<char> {
public:
    string name;
    char symbol;
    FourInARowBoard* boardPtr;
    MinimaxPlayer(string name, char symbol) : Player<char>(name, symbol) {}
    void set_board(FourInARowBoard* board) { boardPtr = board; }
    void getmove(int& x, int& y) override {
        if (!boardPtr) throw runtime_error("Board pointer is null!");
   ...
   int evaluate_board(const vector<vector<char>>& board, char AI, char opponent) {
    ...
    for (int i = 0; i < board.size(); ++i) {
        for (int j = 0; j < board[i].size(); ++j) {
            if (board[i][j] == AI) score += central_weight[j];
            if (board[i][j] == opponent) score -= central_weight[j];
        }
    }
    ...
    auto evaluate_window = [&](const vector<char>& window) -> int {
        ...
        if (AI_count == 3 && opp_count == 0) return 100;  
        if (AI_count == 2 && opp_count == 0) return 10;   
        if (opp_count == 3 && AI_count == 0) return -100; 
        if (opp_count == 2 && AI_count == 0) return -10;  
    ...
    int minimax(vector<vector<char>>& board, int depth, bool isMaximizing, int alpha, int beta, char AI, char opponent, int rows, int columns) {
        if (depth == 0) return evaluate_board(board, AI, opponent);
...
int main() {
    ...
    if (mode == 1) {
       ...
       player2 = new MinimaxPlayer("AI", 'O');
        dynamic_cast<MinimaxPlayer*>(player2)->set_board(&board);
    } else {
        player2 = new randomPlayer("Player 2", 'O');
    }
